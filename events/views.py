...
class EventViewSet(viewsets.ModelViewSet):
    ...
    def import_xlsx(self, request):
        ...
        try:
            ...
            return Response({"message": f"Успешно импортировано: {count}"}, status=status.HTTP_201_CREATED)
        except Exception as e:
            return Response({"error": f"Ошибка при импорте: {str(e)}"}, status=status.HTTP_400_BAD_REQUEST)
