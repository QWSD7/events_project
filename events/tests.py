...
from .models import Event, Location, WeatherData, EventImage
...
class EventLogicTest(TestCase):
    def setUp(self):
        ...
        self.location = Location.objects.create(name="Красноярск", latitude=56.010543, longitude=92.852581)
...
class EventAPITest(APITestCase):
    def setUp(self):
        ...
        self.location = Location.objects.create(name="Дивногорск", latitude=55.957726, longitude=92.380148)
        ...
        Event.objects.create(
            title="Public", status='published', author=self.admin,
            location=self.location, pub_date=aware_dt,
            start_date=aware_dt, end_date=aware_dt
        )
        # Черновик
        Event.objects.create(
            title="Draft", status='draft', author=self.admin,
            location=self.location, pub_date=aware_dt,
            start_date=aware_dt, end_date=aware_dt
        )
